...
| IncInit :
  increment_init ({| Locked := false; Global := O |}, Lock).
...
| Inc2Inv : forall pr1 pr2,
  ...
  -> increment2_invariant (shared_from_private pr1 pr2, pr1, pr2).
...
Proof.
  destruct 1.
  inversion H.
  (** [inversion] is a key tactic.  It is like [destruct], but it applies when one or more arguments to the inductive predicate aren't just variables.  [inversion] figures out which rules could have been used to derive the premise, giving us one new subgoal per rule. *)
  inversion H0.
  inversion 1.
  reflexivity.
Qed.
...
Proof.
  ...
  destruct pr2; inversion H0; subst; inversion 1; subst; apply Inc2Inv'; simpl in *; intuition; subst;
